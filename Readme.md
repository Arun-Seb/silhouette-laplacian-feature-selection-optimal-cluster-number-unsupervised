...
- ✅ **Fully unsupervised** - No ground truth labels required for feature selection
- ✅ **Automatic feature selection** - Combines silhouette analysis, uniformity testing, and Laplacian score
- ✅ **Built-in validation** - Silhouette analysis for cluster quality assessment
- ✅ **Patient-level analysis** - Identifies predominant patterns across multiple events
- ✅ **Comprehensive visualization** - Multiple plots showing selection process and results
- ✅ **Well-documented code** - Extensive comments explaining each step
- ✅ **Synthetic data generator** - Test the algorithm without real data
- ✅ **Ready to use** - Complete working example included
...
├── README.md
├── LICENSE
├── OptimalClusterFeatureSelection.m    % Main analysis script
├── SampleDataGenerator.m                % Generate synthetic OSA data
├── fsulaplacian.m                       % Laplacian Score function
├── data/
│   └── generatedData.mat                % Synthetic data (auto-generated)
└── results/
    └── optimal_feature_selection_results.mat  % Output (auto-generated)
```
...
|----------|------------|-------------|
| `sampleData` | n_samples × 57 | Feature matrix (56 features + 1 label column) |
| `data_len` | 1 × n_patients | Number of events per patient |
| `PSOC_M` | 1 × n_patients | Patient-level ground truth labels ('T'=Tongue, 'N'=Non-tongue) |

**Feature Structure (columns 1-56):**
- **Time-domain**: Energy, Entropy, Zero-crossing rate
- **Formants**: F1, F2, F3
- **MFCC**: 13 coefficients (columns 7-19)
- **ΔMFCC**: 13 first derivatives (columns 20-32)
...
| `opt` | Performance metrics array (6 × n_features) |
...
sampleData = [your_features, your_labels];  % n_samples × (n_features + 1)
data_len = [events_per_patient];            % 1 × n_patients
PSOC_M = {'T', 'N', 'T', ...};             % 1 × n_patients (optional)
...
- **Patient-level predominance**: ≥60% rule enforced

### Output File

```matlab
% generatedData.mat contains:
% - sampleData: [1807 × 57] double
% - data_len: [1 × 54] double  
% - PSOC_M: {1 × 54} cell array of 'T' or 'N'
```
...
🍴 **Fork** to contribute or customize for your research

**Made with ❤️ for the sleep research and machine learning community**